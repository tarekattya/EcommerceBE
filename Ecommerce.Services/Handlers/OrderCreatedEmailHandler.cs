...
public class OrderCreatedEmailHandler(IEmailService emailService) : IDomainEventHandler<OrderCreatedEvent>
{
    ...
    public async Task Handle(OrderCreatedEvent domainEvent)
    {
        ...
        var templatePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Templates", "OrderConfirmation.html");
        ...
        await _emailService.SendEmailAsync(order.BuyerEmail, $"Order #{order.Id} confirmed", emailBody);
    }
}
